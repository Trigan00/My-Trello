...
;(EventEmitter.prototype.on = function (e, t) {
	...
	(EventEmitter.prototype.removeListener = function (e, t) {
		...
		'object' == typeof this.events[e] &&
			(i = this.indexOf(this.events[e], t)) > -1 &&
	...
	(EventEmitter.prototype.emit = function (e) {
		...
		if ('object' == typeof this.events[e])
			for (a = (i = this.events[e].slice()).length, t = 0; t < a; t++)
				i[t].apply(this, s)
	}),
	(EventEmitter.prototype.once = function (e, t) {
		this.on(e, function i() {
			this.removeListener(e, i), t.apply(this, arguments)
		})
	})
var loadScript = function (e, t, i) {
		return new Promise((a, s) => {
			var r = document.createElement('script')
			for (var [l, n] of ((r.async = !0), (r.src = e), Object.entries(t || {})))
				r.setAttribute(l, n)
			;(r.onload = () => {
				;(r.onerror = r.onload = null), a(r)
			}),
				(r.onerror = () => {
					;(r.onerror = r.onload = null), s(new Error(`Failed to load ${e}`))
				}),
				(
					i ||
					document.head ||
...
YouTubePlayer = class extends EventEmitter {
		...
		this.destroyed ||
			...
			this._api &&
				(this._player
					? this._ready &&
						(t
							? this._player.loadVideoById(e, i)
							: this._player.cueVideoById(e, i))
	...
	replayFrom(e) {
		!this.replayInterval.find(
			e => e.iframeParent === this._player.i.parentNode
		) &&
			e &&
				...
				interval: setInterval(
					() => {
						if (
							...
							this._player.getDuration() - Number(e)
						) {
							this.seek(0)
							for (const [e, t] of this.replayInterval.entries())
								Object.hasOwnProperty.call(this.replayInterval, e) &&
									(clearInterval(this.replayInterval[e].interval),
		...
		if (t) {
			...
			if (a) {
				const s = window.getComputedStyle(a),
					r =
						a.clientHeight +
						parseFloat(s.marginTop, 10) +
						parseFloat(s.marginBottom, 10) +
						parseFloat(s.borderTopWidth, 10) +
						parseFloat(s.borderBottomWidth, 10),
					l =
						a.clientWidth +
						parseFloat(s.marginLeft, 10) +
						parseFloat(s.marginRight, 10) +
						parseFloat(s.borderLeftWidth, 10) +
						parseFloat(s.borderRightWidth, 10),
					n = 1.7,
					o = t
				;(e.width = l),
					(e.height = r + 80),
					...
					(o.style.height =
						Math.ceil(parseFloat(o.style.width, 10) / n) + 'px'),
					(o.style.marginTop =
						Math.ceil(-(parseFloat(o.style.height, 10) - e.height) / 2) + 'px'),
					(o.style.marginLeft = 0)
				const h = parseFloat(o.style.height, 10) < e.height
				h &&
					((o.style.height = e.height + 'px'),
					(o.style.width =
						Math.ceil(parseFloat(o.style.height, 10) * n) + 'px'),
					(o.style.marginTop = 0),
					(o.style.marginLeft =
						Math.ceil(-(parseFloat(o.style.width, 10) - e.width) / 2) + 'px'))
				for (const t in i)
					if (i.hasOwnProperty(t)) {
						switch (i[t].replace(/ /g, '')) {
							case 'top':
								o.style.marginTop = h
									? -(parseFloat(o.style.height, 10) - e.height) / 2 + 'px'
									: 0
								break
							case 'bottom':
								o.style.marginTop = h
									? 0
									: -(parseFloat(o.style.height, 10) - e.height) + 'px'
								break
							case 'left':
								o.style.marginLeft = 0
								break
							case 'right':
								o.style.marginLeft = h
									? -(parseFloat(o.style.width, 10) - e.width)
									: '0px'
								break
							default:
								parseFloat(o.style.width, 10) > e.width &&
									(o.style.marginLeft =
										-(parseFloat(o.style.width, 10) - e.width) / 2 + 'px')
						}
					}
			}
		}
	...
	stopReplay(e) {
		for (const [t, i] of this.replayInterval.entries())
			Object.hasOwnProperty.call(this.replayInterval, t) &&
				e === this.replayInterval[t].iframeParent &&
	...
	setSize(e, t) {
		this._ready
			? this._player.setSize(e, t)
			: this._queueCommand('setSize', e, t)
	}
	setPlaybackRate(e) {
		this._ready
			? this._player.setPlaybackRate(e)
			: this._queueCommand('setPlaybackRate', e)
	}
	setPlaybackQuality(e) {
		this._ready
			? this._player.setPlaybackQuality(e)
			: this._queueCommand('setPlaybackQuality', e)
	}
	getPlaybackRate() {
	...
	_loadIframeAPI(e) {
		...
		loadIframeAPICallbacks.push(e),
			Array.from(document.getElementsByTagName('script')).some(
				e => e.src === YOUTUBE_IFRAME_API_SRC
			) ||
		...
		if (!this.destroyed) {
			...
			this._player = new this._api.Player(this._id, {
				...
				playerVars: {
					...
					cc_lang_pref:
						null != t.captions && !1 !== t.captions ? t.captions : void 0,
	...
	_onError(e) {
		if (!this.destroyed) {
			...
			if (t !== YOUTUBE_ERROR.HTML5_ERROR)
				return t === YOUTUBE_ERROR.UNPLAYABLE_1 ||
					t === YOUTUBE_ERROR.UNPLAYABLE_2 ||
					t === YOUTUBE_ERROR.NOT_FOUND ||
					t === YOUTUBE_ERROR.INVALID_PARAM
					? this.emit('unplayable', this.videoId)
					: void this._destroy(
							new Error('YouTube Player Error. Unknown error code: ' + t)
						)
		}
	}
